...
public class DES3 {
	 ...
	 * @throws Exception
	 */
	public static String decode(String encryptText,String desecretKey) throws Exception {
		Key deskey = null;
		DESedeKeySpec spec = new DESedeKeySpec(desecretKey.getBytes());
		SecretKeyFactory keyfactory = SecretKeyFactory.getInstance("desede");
		deskey = keyfactory.generateSecret(spec);
		Cipher cipher = Cipher.getInstance("desede/CBC/PKCS5Padding");
		IvParameterSpec ips = new IvParameterSpec(iv.getBytes());
	...
	// String plainText = "来自http://my.oschina.net/penngo的博�?";
