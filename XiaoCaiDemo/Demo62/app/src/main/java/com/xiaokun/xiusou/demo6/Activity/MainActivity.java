...
import com.xiaokun.xiusou.demo6.Utils.CommonAdapter;
import com.xiaokun.xiusou.demo6.Utils.CommonHolder;
...
//        recyclerView.setAdapter(adapter);//设置adapter
       final CommonAdapter adapter= new CommonAdapter<NewsData.NewsDetails>(response, MainActivity.this, R.layout.item_home) {
            @Override
            public void convert(CommonHolder holder, NewsData.NewsDetails details) {
                Picasso.with(MainActivity.this).load(details.getImg()).fit().into((ImageView) holder.getView(R.id.imageview));
                Picasso.with(MainActivity.this).setIndicatorsEnabled(true);
                ((TextView)holder.getView(R.id.id_num)).setText(details.getContent());
            }

        };
        recyclerView.setAdapter(new CommonAdapter<NewsData.NewsDetails>(response, MainActivity.this, R.layout.item_home) {
            @Override
            public void convert(CommonHolder holder, NewsData.NewsDetails details) {

                PicassoUtil.loadImageViewHolder(MainActivity.this,details.getImg(),(ImageView) holder.getView(R.id.imageview));
//                Picasso.with(MainActivity.this).load(details.getImg()).fit().into((ImageView) holder.getView(R.id.imageview));
                Picasso.with(MainActivity.this).setIndicatorsEnabled(true);
                ((TextView)holder.getView(R.id.id_num)).setText(details.getContent());
            }
